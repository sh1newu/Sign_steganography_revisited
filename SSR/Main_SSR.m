...
for payload=0.01:0.01:0.01
    ...
    stego_dir=fullfile(['E:\Sign_steganography_revisited\SSR\stego_',num2str(payload),'\']); if ~exist(stego_dir,'dir'); mkdir(stego_dir); end
    cover_num=100;
    ...
    if ~exist('E:\Sign_steganography_revisited\SSR\location_map','dir'); mkdir('E:\Sign_steganography_revisited\SSR\location_map'); end %location_map所在的文件夹
    %-------------------------------------------------------------------------------
    parfor i_img=1:cover_num
        try
            ...
            %[ stc_decoded_msg ] = ssrextract( stego_attack_65_path,msg_length(i_img,1),Map{i_img,1} );
            ...
            %-----------------------------------------------------------------------------
            parsave(['E:\Sign_steganography_revisited\SSR\location_map\location_',num2str(payload),'_',num2str(i_img),'.mat'],location_map);
