...
for payload=0.01:0.01:0.01
    ...
    stego_dir=fullfile(['stego_',num2str(payload),'\']); %if ~exist(stego_dir,'dir'); mkdir(stego_dir); end
    cover_num=100;
    ...
    load(['E:\Sign_steganography_revisited\SS\L_',num2str(payload),'.mat'])%msg_length=zeros(cover_num,1);
    %-------------------------------------------------------------------------------
    parfor i_img=1:cover_num
        try
            ...
            %提取信息60
            %  STC extraction
            %[ stc_decoded_msg ] = ssextract( stego_path,msg_length(i_img,1),location_map );
